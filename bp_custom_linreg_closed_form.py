...
class linearRegression():
  ...
  def forward(self,X,y,W):
      y_pred = np.dot(X, W)
      loss = np.mean((y_pred - y)**2)  # Calculate mean squared error (loss function)
      return loss, y_pred              # the prediction and how off the prediction was

  def updateWeights(self,X,y_pred,y_true,W,learning_rate,index):
    for i in range(X.shape[1]):
      W[i] -= (learning_rate * (y_pred-y_true[index])*X[index][i]) 
    return W
  ...
  def train(self, X, y, training_steps=1000, learning_rate=0.001, random_state=0):
    ...
    for j in range(training_steps):
      ...
      for i in train_indices:
        ...
        cost += loss
        W[0] = self.updateWeights(X,y_pred,y,W[0],learning_rate,i)
  ...
  def plotLoss(self, loss, training_steps):
